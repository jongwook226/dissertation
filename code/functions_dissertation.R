...
Cov_fn10 <- function(dat1, dat2, a1, a2, a3, tau){
  ...
  R <- a3[1]*ICF(SphDist(lat1,long1, lat2, long2),1) + a3[2]*(ICF(0,2)+1) - a3[3]*ICF(SphDist(lat1,long1,tau[1],tau[2]),3) - a3[3]*ICF(SphDist(lat2,long2,tau[1],tau[2]),3)
...
Cov_fn20 <- function(dat1, dat2, a1, a2, a3, tau_lat, tau_long, q1, q2, dist_taus){
  # tau_lat <- tau$tau_lat; tau_long <- tau$tau_long
  # i=1;j=50;dat1 <- dat[i,]; dat2 <- dat[j,]; tau_lat <- tau$tau_lat; tau_long <- tau$tau_long; q1=q[i,]; q2=q[j,]
  ...
  Nil2 <- matrix(nrow=length(q1), ncol=length(q2))
  Nil22 <- 0
  Nil3 <- c(as.matrix(q2))
  Nil4 <- c(as.matrix(q1))
  ...
  for(i in 1:length(q1)){
    for(j in 1:length(q2)){
     ...
     if(i == j){ #when nu = mu
       Nil22 <- Nil22 + Nil4[i]*Nil3[j]
     }
    }
  }
  Nil2 <- c(as.matrix(Nil2))
  
  R <- (a3[1]*(icf1 - trunc1)) + (a3[2] * (sum((icf2 - trunc2) * Nil2) + Nil22)) - (a3[3] * sum((icf3 - trunc3) * Nil3)) - (a3[3] * sum((icf4 - trunc4) * Nil4))
...
Cov_mat <- function(dat, a1, a2, a3, kappa, tau_lat, tau_long, q){
  #tau_lat <- tau$tau_lat; tau_long <- tau$tau_long
  ...
  for(i in 1:N){
    for(j in i:N){
      ...
      if(kappa == 0){
        ...
        R[i,j] <- R[j,i] <- Cov_fn20(dat[i,], dat[j,], a1, a2, a3, tau_lat, tau_long, q[i,], q[j,], dist_taus)
      }
    }
  }
  return(R)
...
C_Cov_mat <- function(dat=dat, sDmat, a1, a2, a3, kappa, q, tauDist, bwtauDist){
  
  dat$index = dat$index-1 #for C
  nrow_dat <- nrow(dat); ncol_dat <- ncol(dat) #data set
  nrow_Dmat <- nrow(sDmat) #spherical distance matrix
  ncol_q <- ncol(q) # the number of truncated spherical harmonics (kappa)
  ...
  res <- .C('Cov_mat', as.integer(nrow_dat), as.integer(ncol_dat), as.double(as.matrix(dat)), as.integer(nrow_Dmat),
            as.double(sDmat), as.double(a1), as.double(a2), as.double(a3), as.integer(kappa), as.double(as.matrix(q)), as.integer(ncol_q), as.double(as.matrix(tauDist)), as.double(as.matrix(bwtauDist)), R=as.double(R))
